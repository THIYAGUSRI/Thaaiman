...
export default function AdminEditProductPage() {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const formattedRates = product.prod_Rate && product.prod_Rate.length > 0
          ? product.prod_Rate.map(rate => ({
            size: Object.keys(rate)[0],
            price: rate[Object.keys(rate)[0]].toString(),
          }))
          : [{ size: '', price: '' }];
        ...
        setFormData({
          ...
          prod_Rate: formattedRates,
          prod_Stock: product.prod_Stock || 0,
          prod_active: product.prod_active !== undefined ? product.prod_active : true,
          prod_Description: product.prod_Description || '',
          prod_offer: product.prod_offer || 0,
          sortOrderList: product.sortOrderList || 0,
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.prod_Name) data.append('prod_Name', formData.prod_Name);
      if (formData.prod_category) data.append('prod_category', formData.prod_category);
      if (formData.prod_Stock) data.append('prod_Stock', formData.prod_Stock);
      if (formData.prod_active !== undefined) data.append('prod_active', formData.prod_active);
      if (formData.prod_Description) data.append('prod_Description', formData.prod_Description);
      if (formData.prod_offer !== undefined) data.append('prod_offer', formData.prod_offer);
      if (formData.sortOrderList !== undefined) data.append('sortOrderList', formData.sortOrderList);

      // Send images to delete if any
      if (imagesToDelete.length > 0) {
        data.append('deletedImages', JSON.stringify(imagesToDelete));
      }
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                      {existingImages.map((image, index) => (
                        <div key={index} className="relative group">
                          <img src={image.url} alt={`Existing ${index}`} className="rounded-md h-24 w-full object-cover" />
                          <button
                            type="button"
